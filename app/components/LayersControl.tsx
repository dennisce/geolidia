...
import type { ColorScaleKey, FeatureTipo, UfCode } from "./tiles.helpers"
import { getVisualizationLabel, getColorScaleColors, getColorScaleLabel, UF_OPTIONS } from "./tiles.helpers"
      ...
      >
        ...
        <div className="border-b border-gray-100 px-4 py-4">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-gray-900">Configuração de território</span>
            <HelpTip text="Define a UF e o tipo de geometria exibida no mapa." />
          </div>
          ...
          <div className="mt-3 space-y-3">
            <div>
              ...
              <select
                value={uf}
                onChange={(e) => onChangeUf(Number(e.target.value) as UfCode)}
                  ...
                  <div className="space-y-3 border-t border-gray-100 px-3 pb-3 pt-2">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 outline-none focus:border-violet-400"
                      >
                        {getColorScaleColors(layer.colorScale).map((color, index) => (
                          <option key={index} value={color}>
                            {getColorScaleLabel(layer.colorScale)}
                          </option>
                        ))}
                      </select>
                    </div>
